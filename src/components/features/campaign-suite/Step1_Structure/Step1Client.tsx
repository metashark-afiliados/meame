...
export function Step1Client({ content }: Step1ClientProps): React.ReactElement {
  const traceId = useMemo(() => logger.startTrace("Step1Client_Lifecycle_v9.0"), []);
  useEffect(() => {
    logger.info("[Step1Client] Orquestador de cliente montado.", { traceId });
    return () => logger.endTrace(traceId);
  }, [traceId]);

  const { headerConfig, footerConfig, updateHeaderConfig, updateFooterConfig } = useStep1StructureStore();
  const { completeStep } = useDraftMetadataStore();
  const wizardContext = useWizard();

  const handleHeaderConfigChange = useCallback((newConfig: Partial<HeaderConfig>) => {
    logger.traceEvent(traceId, "Acción: Actualizando config de header.", newConfig);
    updateHeaderConfig(newConfig);
  }, [updateHeaderConfig, traceId]);

  const handleFooterConfigChange = useCallback((newConfig: Partial<FooterConfig>) => {
    logger.traceEvent(traceId, "Acción: Actualizando config de footer.", newConfig);
  ...
  if (!wizardContext) {
    ...
    return <DeveloperErrorDisplay context="Step1Client" errorMessage={errorMsg} />;
  }
