...
export function Step5Client({ locale, stepContent }: Step5ClientProps): React.ReactElement {
  const traceId = useMemo(() => logger.startTrace("Step5Client_Lifecycle_v11.0"), []);
  useEffect(() => {
  ...
  const { onPublish, onPackage, onDelete, isPublishing, isPackaging, isDeleting } = useCampaignLifecycle(locale, assembledDraft);
  const { onSaveAsTemplate, isSavingTemplate } = useCampaignTemplates(assembledDraft);
  ...
  const isLaunchReady = useMemo(() => checklistItems.every((item) => item.isCompleted), [checklistItems]);
  ...
  const handleDelete = useCallback(() => {
    logger.warn("[Step5Client] Acción de usuario: Iniciar Eliminación de Borrador.", { traceId });
    onDelete();
  }, [onDelete, traceId]);

  const handleSaveTemplate = useCallback((name: string, description: string) => {
    logger.traceEvent(traceId, "Acción de usuario: Guardar como Plantilla.", { name });
    onSaveAsTemplate(name, description);
  }, [onSaveAsTemplate, traceId]);

  if (!stepContent) {
    return <DeveloperErrorDisplay context="Step5Client" errorMessage="Contenido i18n no proporcionado." />;
  }
  if (!assembledDraft.draftId || !activeWorkspaceId) {
    return <DeveloperErrorDisplay context="Step5Client" errorMessage="Faltan datos críticos (draftId o workspaceId)." />;
  }
  ...
  return (
    <>
      <Step5Form
        ...
        artifactHistorySlot={<ArtifactHistory draftId={assembledDraft.draftId} content={stepContent.artifactHistory} />}
      />
