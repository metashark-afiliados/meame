...
if (!ENCRYPTION_KEY || ENCRYPTION_KEY.length < 32) { // La clave debe ser al menos de 32 bytes para AES-256
  logger.error("[Server Encryption] CRÍTICO: La variable de entorno 'SUPABASE_JWT_SECRET' no está definida o es demasiado corta para la encriptación.");
  throw new Error("Missing or insecure 'SUPABASE_JWT_SECRET' for server encryption.");
}
...
export function decryptServerData(encryptedText: string): string {
  ...
  logger.traceEvent(traceId, "Iniciando desencriptación de datos en el servidor.");
  ...
  try {
    ...
    logger.error("Fallo de desencriptación: datos corruptos o clave incorrecta.", { error, traceId });
